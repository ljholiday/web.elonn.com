...
$checks = [
    ...
    'Runtime requests only the World Dataset session endpoint' => str_contains($scripts, "postJson('/world/session'")
        && str_contains($scripts, "contract: {\n                    name: 'elonn.world.session_request'")
        && !str_contains($scripts, '/world/runtime')
        && !str_contains($scripts, '/world/translated')
        && !str_contains($scripts, 'provider_source_profile'),
    'Runtime dispatches only published World action endpoints' => str_contains($scripts, "indexOf('/world/actions/') !== 0")
