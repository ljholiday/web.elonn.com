...
$checks = [
    ...
    'Runtime exposes query composer input and voice control' => str_contains($webRenderer, "form.setAttribute('data-runtime-query-form'")
        ...
        && str_contains($webRenderer, "input.type = 'search'")
    ...
    'Runtime queries include browser origin for World when available' => str_contains($scripts, 'browserOrigin')
        ...
        && !str_contains($scripts, 'needsBrowserOrigin')
        && !str_contains($scripts, 'near me|nearby|around me|close to me|in my area')
        && !str_contains($scripts, 'right now|today')
        && !str_contains($scripts, 'get|buy|find|go|visit|eat|drink|shop|available|open')
        && !str_contains($scripts, '/maps/call'),
    'Runtime capabilities advertise keyboard voice pointer and operation action dispatch' => str_contains($scripts, 'keyboard: true')
    ...
    'Runtime keeps notification areas at the bottom of the viewport' => str_contains($template, 'runtime-notifications')
        ...
        && !str_contains($runtimeCss, "runtime-session {\n    right: 20px;\n    top:"),
    'Workspace results panel is built through the same floatingPanel() function every carry panel uses' => str_contains($template, 'data-workspace-panel-mount')
        && !str_contains($template, 'workspace-layer')
        && !str_contains($template, 'data-runtime-query-form')
        ...
        && str_contains($webRenderer, "config.closable === false ? null : panelButton(")
        && str_contains($webRenderer, 'carry-object-panel__title')
        && str_contains($webRenderer, 'carry-object-panel__actions')
        && str_contains($webRenderer, "dataset: 'workspaceResultsToggle'")
        && str_contains($webRenderer, "dataset: 'workspaceResultsClear'")
        && !str_contains($webRenderer, "node.closest('[data-workspace-results-panel]')")
        && str_contains($webRuntime, "function carryPanel(panelId)")
        && str_contains($webRuntime, "panelId === 'workspace-results'")
        && str_contains($webRuntime, 'function reconcileWorkspacePanel(saved)')
        && str_contains($webRuntime, 'function persistWorkspacePanel()')
        && str_contains($webRuntime, 'function loadWorkspacePanel()')
        && str_contains($webRuntime, "toggleCarryPanel('workspace-results')")
    ...
    'Runtime projects Carry Workspace and Field as environment anchors' => str_contains($template, 'data-field-projection')
        && str_contains($webRenderer, 'query-composer')
        && str_contains($template, 'data-runtime-carry-panels')
        && str_contains($template, 'data-workspace-panel-mount')
